...
class Userbot(Client):
    ...
    async def start(self):
        LOGGER(__name__).info(f"Starting Assistants great...")
        if config.STRING1:
            ...
            assistants.append(1)
            try:
                await self.one.send_message(config.LOGGER_ID, "Assistant great Started")
            except:
        ...
        if config.STRING2:
            ...
            assistants.append(2)
            try:
                await self.two.send_message(config.LOGGER_ID, "Assistant great Started")
            except:
                LOGGER(__name__).error(
        ...
        if config.STRING4:
            ...
            assistants.append(4)
            try:
                await self.four.send_message(config.LOGGER_ID, "Assistant great Started")
            except:
                LOGGER(__name__).error(
        ...
        if config.STRING5:
            ...
            assistants.append(5)
            try:
                await self.five.send_message(config.LOGGER_ID, "Assistant great Started")
            except:
                LOGGER(__name__).error(
